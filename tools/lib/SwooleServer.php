...
class SwooleServer {
    ...
    // 业务启动文件，如果配置了，那么自动在 worker 中加载
    public static $bootFile;
    ...
    public function onStart($server) {
        ...
        $server->masterLog = self::log('master_pid', $server->master_pid, false, true);
        $server->managerLog = self::log('manager_pid', $server->manager_pid, false, true);
    ...
    public function onShutdown($server) {
        ...
        unlink($server->masterLog);
        unlink($server->managerLog);
    ...
    private static function log($logName, $content, $date = true, $overwrite = false) {
        ...
        if ($date) {
            $logFile = self::$_path . '/log/' . $logName . '-' . @date('Y-m-d') .'.log';
        } else {
     ...
     * @return array
     */
    private static function _initProf($server = '') {
